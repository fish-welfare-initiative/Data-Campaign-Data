...
Ponds are identified by anonymised IDs of the form `ara2_xxxxxxxx`. These are the
same identifiers used in our [public ARA dataset](https://github.com/fish-welfare-initiative/fwi-farm-data-india),
so the two datasets can be joined on pond ID.

Earlier versions of this repository used identifiers of the form `pond_xxxxxxxx`.
Those were replaced when the ARA dataset moved to randomly generated identifiers,
and are not comparable with the current ones. If you are working from an older
copy of either dataset, re-download both.
...
Most zero-value readings occur on January 8, 2026, across many devices simultaneously (likely a coordinated calibration or firmware event). A few ponds also have longer stretches of zeros: `ara2_3ed4df8e` has a multi-day outage starting January 5, and `ara2_ca187575` has persistent pH=0 readings after being reinstalled on January 25 following a harvest. These zeros are not flagged by the QC conditional formatting rules but are noted in `csv/QC_Flags.csv`.
...
- All pond identifiers have been anonymised. The Pond IDs are the same as those in our [public ARA dataset](https://github.com/fish-welfare-initiative/fwi-farm-data-india), so the datasets can be cross-referenced. See *Pond identifiers* above.
